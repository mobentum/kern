...
`xvalidator.BodyValidator[T]()` decodes and validates the request body in a single middleware step. The validated struct is stored in the request context and retrieved with `xvalidator.ValidatedFromContext[T]()`.
...
func main() {
    ...
    api.AddConstraints("POST", "/users", kern.Constraints{
        ...
        req, _ := xvalidator.ValidatedFromContext[CreateUserRequest](c.Context())
...
- **Validation middleware**: `xvalidator.BodyValidator[T]()` decodes + validates in one step; `xvalidator.ValidatedFromContext[T]()` retrieves the struct from context
