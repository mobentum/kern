...
## Configuration with config (Optional)

Use the optional `config` module when you want `.env` file loading and typed environment access.
...
## OpenAPI with openapi (Optional)

Use the optional `openapi` module when you want explicit API docs endpoints without reflection.
