# xconfig - dotenv and environment loading for kern

`xconfig` is an optional configuration module for kern for dotenv-style loading and environment access.

- Standalone module: `github.com/mobentum/kern/extensions/xconfig`
...
```go
err := xconfig.Load()
err := xconfig.Load(".env.local", ".env")
err := xconfig.Overload(".env.test")
xconfig.MustLoad(".env")
```
...
func LoadConfig() (*Config, error) {
    loader, err := xconfig.New(xconfig.WithDotEnv())
    if err != nil {
        return nil, err
    }
...
if err != nil {
    switch {
    case errors.Is(err, xconfig.ErrInvalidValue):
        return err
    case errors.Is(err, xconfig.ErrMissingRequired):
        return err
    }
}
```
...
- `extensions/xconfig/examples/kern-integration`

Run it locally:

```bash
cd extensions/xconfig/examples/kern-integration
go run .
```
